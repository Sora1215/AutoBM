...
void XLDataWrapper::CheckForFormula(KR_STR paramFileName)
{
    ...
    if (XLSX->load(paramFileName) == true)
    {
        ...
        if (sheet != nullptr)
        {
            P_STRING("Checking for any formulas in the sheet", C_GREEN);
            ...
            for (int row = sheet->firstRow(); row < sheet->lastRow(); row++)
            {
                for (int col = sheet->firstCol(); col < trueLastColIndex; col++)
                {
                    ...
                    {
                        P_POSITION(row, col, C_BLUE, false);
...
void XLDataWrapper::CheckForZeroWidthSpace(KR_STR paramFileName)
{
    ...
    if (XLSX->load(paramFileName) == true)
    {
        ...
        if (sheet != nullptr)
        {
            P_STRING("Checking for any Zero Width Spaces in the sheet", C_GREEN);
...
void XLDataWrapper::CheckForItemLocal(KR_STR paramFileName)
{
    ...
    if (XLSX->load(paramFileName) == true)
    {
        ...
        P_STRING("~ ItemTable sheet 0 ~", C_GREEN);

        if (ItemTable != nullptr)
        {
            P_STRING("Making sure that all cell contains a number...", C_GREEN);
            ...
            for (int row = ROW_ITEMTABLE_FIRST; row < ItemTable->lastRow(); row++)
            {
                ...
                {
                    P_POSITION(row, COL_ITEMTABLE_ITEMINDEX, C_BLUE, false);
                    LOGIC_PRINTCELLTYPE;
        ...
        P_STRING("~ ItemTable sheet 1 ~", C_GREEN);

        if (LocalTable != nullptr)
        {
            P_STRING("Checking for any non-string data in the sheet...", C_GREEN);
            ...
            for (int row = ROW_ITEMTABLE_FIRST; row < LocalTable->lastRow(); row++)
            {
                ...
                {
                    P_POSITION(row, COL_LOCALTABLE_ITEMINDEX, C_BLUE, false);
                    LOGIC_PRINTCELLTYPE;
                ...
                else
                {
                    ...
                    catch (std::invalid_argument msg)
                    {
                        ...
                        P_POSITION(row, COL_LOCALTABLE_ITEMINDEX, C_BLUE, false);
                        P_STRING(LocalTable->readStr(row, COL_LOCALTABLE_ITEMINDEX), C_BLUE, true);
                    }
                }
            }
        }
        ...
        for (auto elm : mItemTable_ItemIndex_Row)
        {
            if (mLocalTable_ItemIndex_Count.find(elm.first) != mLocalTable_ItemIndex_Count.end())
            {
                ...
                {
                    P_DOUBLE(elm.first, C_PARAMETER, false);
                    P_STRING(") <-> ", C_GREEN, false);
                    P_DOUBLE(mLocalTable_ItemIndex_Count[elm.first], C_PARAMETER);
